...
class Solver(object):
    ...
    def fit(self,
        ...
        mod = mx.mod.Module(symbol=self.symbol,
                            data_names=[train_data.data_name, train_data.avatar, train_data.nickname],
                            label_names=[train_data.label_name],
                              context=self.ctx)

        mod.bind(data_shapes=[('data', (self.batch_size, 3, 224, 224)),
                              ('avatar', (self.batch_size, 50, 300)),
                              ('nickname', (self.batch_size, 50, 300))],
                 label_shapes=[('softmax_label', (self.batch_size, ))])
