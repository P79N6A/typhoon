...
def symbol_unet(data, label=None, num_class=6, BatchNorm=False):
    ...
    conv9_1 = conv_unit(data=k_4, num_filter=32, workspace=2048, name="conv9_1", BatchNorm=BatchNorm)
    conv9_2 = conv_unit(data=conv9_1, num_filter=32, workspace=2048, name="conv9_2", BatchNorm=BatchNorm)
    conv10 = mx.symbol.Convolution(data=conv9_2, kernel=(1, 1), pad=(0, 0), num_filter=num_class, workspace=2048, name="conv10")

    softmax = mx.symbol.SoftmaxOutput(data=conv10, label=label, multi_output=True, use_ignore=True, ignore_label=255, name="softmax")

    return softmax

def get_u_net(name=None,num_class=6):
    data = mx.symbol.Variable('data')
    label = mx.symbol.Variable('softmax')
    if name == 'deconv':
        net = symbol_unet_dc(data, label, BatchNorm=True, num_class=num_class)
    else:
        net = symbol_unet(data, label, BatchNorm=True, num_class=num_class)
    print net.list_arguments()
    arg_shape, output_shape, aux_shape = net.infer_shape(data=(1, 1, 512, 512))
