...
def get_symbol_resnet_fpn(num_class=2, depth=50, loss_type='softmax', bn_mom=0.9, workspace=512):
    ...
    for i in range(len(steps)-1, -1, -1):
        steps[i] = mx.sym.Convolution(data=steps[i], num_filter=int(filter_list[-1] / 4), kernel=(1, 1), stride=(1, 1),
                                      pad=(0, 0), workspace=workspace, name='c_1X1' + str(i))
        if i != len(steps)-1:
            steps[i] = steps[i] + mx.sym.UpSampling(steps[i+1], scale=2, sample_type='nearest', name='u_'+str(i), workspace=workspace)

        con1 = mx.sym.Convolution(data=steps[i], num_filter=int(filter_list[-1] / 4), kernel=(3, 3), stride=(1, 1), pad=(3, 3),
                                  no_bias=True, name='conv1_'+str(i)+'_step', workspace=workspace)

        bn1 = mx.sym.BatchNorm(data=con1, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn1_'+str(i)+'_step')
        relu1 = mx.sym.Activation(data=bn1, act_type='relu', name='relu1_'+str(i)+'_step')
        pool = mx.symbol.Pooling(data=relu1, global_pool=True, kernel=(7, 7), pool_type='avg', name='pool_'+str(i)+'_step')
...
if __name__ == '__main__':
    net = get_symbol_resnet_fpn(num_class=2, depth=101, loss_type='softmax', bn_mom=0.9, workspace=512)