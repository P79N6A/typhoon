...
class FileIter(DataIter):
    ...
    def get_lis(self):
        ...
        for i in range(self.num/self.batch_size):
            ...
            for _ in range(self.batch_size):
                ...
                batch.append([idx, self.img_ids[idx]])
                self.img_ids[idx] += 1
    ...
    def get_batchs(self, integer):
        ...
        while True:
            if not self.lines_queue.full():
                batch_lis = self.lines_queue.get(block=True, timeout=2)
    ...
    def reset(self):
        ...
        print "num:", self.lines_queue.qsize()
...
if __name__ == "__main__":
    ...
    i = 0
    while True:
        print i, val_dataiter.imgs_queue.qsize()
        i += 1
        if val_dataiter.iter_next():
            val = val_dataiter.next()
            print "%%%", val
        else:
            raise StopIteration
